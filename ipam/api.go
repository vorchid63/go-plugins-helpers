...
import (
	...
	"github.com/vorchid63/go-plugins-helpers/sdk"
)
...
const (
	...
	requestVZAddressPath = "/IpamDriver.RequestVZAddress"
	releaseVZAddressPath = "/IpamDriver.ReleaseVZAddress"
)
...
type Ipam interface {
	...
	RequestTTAddress(*RequestTTAddressRequest) (*RequestAddressResponse, error)
	ReleaseAddress(*ReleaseAddressRequest) error
	RequestVZAddress(*RequestVZAddressRequest) (*RequestAddressResponse, error)
	ReleaseVZAddress(*ReleaseVZAddressRequest) error
}
...
// RequestVZAddressRequest is sent when requesting an address from IPAM
type RequestVZAddressRequest struct {
	PoolID   string
	Address  string
	TenantID int8
	Options  map[string]string
...
// RequestTTAddressRequest is sent when requesting an address from IPAM
type RequestTTAddressRequest struct {
	PoolID  string
	Address string
	Options map[string]string
}
...
// ReleaseVZAddressRequest is sent in order to release an address from the pool
type ReleaseVZAddressRequest struct {
	PoolID   string
	Address  string
	TenantID int8
}
...
func (h *Handler) initMux() {
	...
	h.HandleFunc(requestAddressPath, func(w http.ResponseWriter, r *http.Request) {
		req := &RequestTTAddressRequest{}
		err := sdk.DecodeRequest(w, r, req)
		if err != nil {
			return
		}
		res, err := h.ipam.RequestTTAddress(req)
		if err != nil {
			msg := err.Error()
			sdk.EncodeResponse(w, NewErrorResponse(msg), msg)
			return
	...
	h.HandleFunc(requestVZAddressPath, func(w http.ResponseWriter, r *http.Request) {
		req := &RequestVZAddressRequest{}
		err := sdk.DecodeRequest(w, r, req)
		if err != nil {
			return
		}
		res, err := h.ipam.RequestVZAddress(req)
		if err != nil {
			msg := err.Error()
			sdk.EncodeResponse(w, NewErrorResponse(msg), msg)
			return
	...
	h.HandleFunc(releaseVZAddressPath, func(w http.ResponseWriter, r *http.Request) {
		req := &ReleaseVZAddressRequest{}
		err := sdk.DecodeRequest(w, r, req)
		if err != nil {
			return
		}
		err = h.ipam.ReleaseVZAddress(req)
		if err != nil {
			msg := err.Error()
			sdk.EncodeResponse(w, NewErrorResponse(msg), msg)
		}
